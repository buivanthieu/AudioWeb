...
{
    public class GetAllOriginalStoriesHandler : IRequestHandler<GetAllOriginalStoriesQuery, IEnumerable<OriginalStoryListDto>>
    {
        ...
        public async Task<IEnumerable<OriginalStoryListDto>> Handle(GetAllOriginalStoriesQuery request, CancellationToken cancellationToken)
        {
            var originalStories = await _originalStoryRepository.GetAllAsync();
            return _mapper.Map<IEnumerable<OriginalStoryListDto>>(originalStories);
        }
    }
}
